...
/* ---------- 4. the ideas section, top of the Activity tab ----------
   Two possible starting states: the Wave 6 file (enrichCardHTML call present)
   or a pristine pre-Wave-6 file (plain casting-card line). Handle both. */
var CAST_ANCHOR = 'if(c.J){\n    var _pool = jobPool(c.k, c.a);';
if(s.indexOf('h += ideasHTML(c);') > -1){
  skipped.push("4. ideas section rendered before the casting card");
} else if(s.indexOf('h += enrichCardHTML(c);\n  ' + CAST_ANCHOR) > -1){
  rep("4. ideas section replaces the enrich card",
    'h += enrichCardHTML(c);\n  ' + CAST_ANCHOR,
    'h += ideasHTML(c);\n  ' + CAST_ANCHOR);
} else {
  rep("4. ideas section rendered before the casting card",
    '  ' + CAST_ANCHOR,
    '  h += ideasHTML(c);\n  ' + CAST_ANCHOR);
}

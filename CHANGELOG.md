...
| `5.0.0-dev.7` | 18 Aug | The writer-asks-first chat: before writing, one small call reads the intake and asks up to 3 clarity questions (a clean intake asks none and writing starts alone); answers ride into the idea prompt with professor authority; a rejected run offers "Let the writer ask you what to fix" seeded with the rule failures; a failed clarity pass is skipped, never blocking. The free-text note box stays alongside. Plus the SNHU AI & data guidance line — masthead on both surfaces, the chat turn, and the note box: public information only |
